...
impl Client<roles::Registered> {
    ...
    pub async fn create_material_usage(
        ...
        self.execute(Statements::CreateMaterialUsage, &[&user_id, &material_id])
            .await?;
        Ok(())
    }

    pub async fn insert_material_rating(
        &self,
        material_id: MaterialKey,
        user_id: UserKey,
        rating: &UserRating,
        ...
        self.update_one(
            Statements::UpdateMaterialRating,
            &[&material_id, &user_id, &rating.rating],
        )
        .await
    }
}
    ...
    async fn init_statements(client: &PostgresClient) -> InitStatementsResult<Statements> {
        let mut statements = enum_map! {
            Statements::PostReviewWithId => client.prepare_typed(
                ...
                .map_err(|error| InitStatementsError {statement_key: Statements::PostReviewWithId, error})?,
            Statements::CreateMaterialUsage => client.prepare_typed(
                ...
                .map_err(|error| InitStatementsError {statement_key: Statements::CreateMaterialUsage, error})?,
            Statements::UpdateMaterialRating => client.prepare_typed(
                ...
                .map_err(|error| InitStatementsError {statement_key: Statements::UpdateMaterialRating, error})?,
        };
